...
try{
 ...
 // Intended V1 rule: legacy claim writes are retired (typed progress_claims are the single money store; duplicate-claim protection is covered by test:v1 scenario E).
 r=await call('/api/commercial','POST',{action:'claim',jobId:job.id,docketIds:['legacy-2'],claimPeriod:'2026-09'},a.cookie);assert.equal(r.status,410,await r.clone().text());const [legacyClaims]=await db.execute("SELECT COUNT(*) AS n FROM claim_items WHERE docket_id='legacy-2'");assert.equal(Number(legacyClaims[0].n),0,'no legacy claim rows');
 ...
 console.log('PASS MySQL estimate → award → job, immutable approved budget, foreign write rejection, legacy claim writes retired, per-tenant default rates, concurrent tenant context');
