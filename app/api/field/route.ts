...
async function handlePOST(request:Request){try{
 ...
 const data=!user.priced?preserveFieldPricing(input,previous?.data||initialField(plan)):input;
 if(user.priced)data.pricingReviewRequired=!data.materialCost||!data.otherCost||data.resources.some(r=>r.rate===undefined||r.rate==='')||data.variations.some(r=>r.cost===undefined||r.cost==='');
 const errors=body.action==='save'?invalidField(data):incomplete(data,user.priced);
 ...
 return Response.json({record:!user.priced?fieldRecord(saved):saved,user},{status:previous?200:201});
