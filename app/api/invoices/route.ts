...
export async function POST(request:Request){try{
 ...
 const duplicate=draft.invoiceNumber&&draft.supplier?await db.prepare("SELECT id FROM attachments WHERE organisation_id=? AND json_extract(metadata,'$.kind')='invoice' AND lower(json_extract(metadata,'$.draft.invoiceNumber'))=lower(?) AND lower(json_extract(metadata,'$.draft.supplier'))=lower(?) LIMIT 1").bind(actor.organisationId,draft.invoiceNumber,draft.supplier).first():null;
...
export async function PUT(request:Request){try{
 ...
 const row=await db.prepare("SELECT metadata,status FROM attachments WHERE organisation_id=? AND id=? AND json_extract(metadata,'$.kind')='invoice'").bind(actor.organisationId,body.id).first<{metadata:string;status:string}>();if(!row)return Response.json({error:'Invoice not found'},{status:404});
 ...
 const duplicate=counterparty&&draft.invoiceNumber?await db.prepare("SELECT id FROM attachments WHERE organisation_id=? AND id!=? AND json_extract(metadata,'$.kind')='invoice' AND lower(trim(json_extract(metadata,'$.draft.invoiceNumber')))=lower(trim(?)) AND json_extract(metadata,'$.draft.direction')=? AND lower(trim(json_extract(metadata,?)))=lower(trim(?)) LIMIT 1").bind(actor.organisationId,body.id,draft.invoiceNumber,draft.direction,draft.direction==='Customer invoice'?'$.draft.customer':'$.draft.supplier',counterparty).first():null;
