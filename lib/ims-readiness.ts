...
export async function imsBlockers(db:Database,organisationId:string,jobId:string){
 ...
 return [...(preparation||[]),...items.results.filter(i=>Number(i.mandatory)===1&&i.status!=='Not Applicable'&&(!['Approved','Accepted'].includes(String(i.status))||!['Approved','Accepted'].includes(String(i.document_status))||!i.attachment_id||(i.expiry_date&&String(i.expiry_date)<today)||(i.effective_date&&String(i.effective_date)>today))).map(i=>`IMS: ${i.title} requires current approved evidence.`)];
}
