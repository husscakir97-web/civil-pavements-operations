...
import { DEFAULT_ORGANISATION_ID, jsonError, nowIso, requireEstimateDb, safeJson } from "@/lib/estimates-db";
...
export async function POST(request: Request) {
  try {
    ...
    const opportunity = await db.prepare("SELECT id,name,status,metadata FROM opportunities WHERE organisation_id=? AND id=?").bind(DEFAULT_ORGANISATION_ID, opportunityId).first<{id:string;name:string;status:string;metadata:string}>();
    ...
    await db.batch([
      db.prepare("INSERT INTO estimates (id,organisation_id,name,status,metadata,created_at) VALUES (?,?,?,?,?,?)").bind(estimateId, DEFAULT_ORGANISATION_ID, data.name || opportunity.name, "Draft", JSON.stringify(estimateMeta), now),
      db.prepare("INSERT INTO quote_revisions (id,organisation_id,name,status,metadata,created_at) VALUES (?,?,?,?,?,?)").bind(revisionId, DEFAULT_ORGANISATION_ID, `${opportunity.name} · Rev 1`, "Draft", JSON.stringify({ estimateId, revisionNumber: 1, data, totals, validation, reason: "Converted from opportunity" }), now),
      db.prepare("UPDATE opportunities SET metadata=? WHERE organisation_id=? AND id=?").bind(JSON.stringify({ ...meta, convertedEstimateId: estimateId, convertedAt: now }), DEFAULT_ORGANISATION_ID, opportunityId),
      db.prepare("INSERT INTO audit_events (id,organisation_id,name,status,metadata,created_at) VALUES (?,?,?,?,?,?)").bind(crypto.randomUUID(), DEFAULT_ORGANISATION_ID, `opportunity.converted:${opportunityId}`, "recorded", JSON.stringify({ estimateId }), now),
    ]);
