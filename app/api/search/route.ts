...
const specs:SearchSpec[]=[
 {table:"clients",type:"Client",name:"name",status:"'active'",detail:"CONCAT_WS(' ',contact_name,email,phone)",metadata:"'{}'"},
 ...
 {table:"dockets",type:"Docket",name:"docket_no",status:"status",detail:"CONCAT_WS(' ',client,project,po_number,notes)",metadata:"'{}'",filter:"AND lower(status) != 'archived'"},
];
...
async function handleGET(request:Request){
 try{
  ...
  const groups=await Promise.all(specs.map(async spec=>{
   ...
   return r.results.map(x=>({...x,type:spec.type,metadata:safeJson(x.metadata,{})}));
  }));
