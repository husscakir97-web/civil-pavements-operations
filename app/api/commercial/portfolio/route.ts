...
export const GET=api({permission:'read',module:'commercial',capability:'commercial.view'},async({actor})=>{
 ...
 const rows=[];for(const p of projects){const f=await projectFinancials(p.id);rows.push({id:p.id,name:p.name,projectNumber:p.project_number,clientName:p.client_name,stage:stageOf(p),hasBaseline:f.hasBaseline,...f.forecast,retentionHeld:f.retention.held});}
