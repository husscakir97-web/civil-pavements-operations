...
export async function preparationRecords(db:Database,org:string){const r=await db.prepare('SELECT * FROM preparation_revisions WHERE organisation_id=? ORDER BY created_at,revision').bind(org).all<Omit<PreparationRecord,'data'>&{data:string}>();return r.results.map(r=>({...r,data:dataSchema.parse(JSON.parse(r.data))}));}
...
export async function checkLinks(db:Database,actor:Actor,record:PreparationRecord,records:PreparationRecord[]){
...
export function appendStatements(db:Database,record:PreparationRecord){return [
...
export async function preparationReadiness(db:Database,org:string,jobId:string){
