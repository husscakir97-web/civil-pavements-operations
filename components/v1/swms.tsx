...
function SwmsDetail({id,shiftId,onChanged}:{id:string;shiftId?:string;onChanged:()=>void}){
 ...
 const {data,error,loading,refresh}=useApi<Detail>(`/api/hseq/swms?id=${id}`);
 ...
 return <div className="grid gap-4 p-5">
  ...
  <div className="flex flex-wrap gap-2">
   ...
   {issued&&can('swms.acknowledge')&&<Btn busy={busy} onClick={()=>void act({action:'acknowledge',shiftId:shiftId||null})}><FileSignature aria-hidden className="size-4"/>I have read and understood this SWMS</Btn>}
  </div>
