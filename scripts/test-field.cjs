...
function req(body,email='huss.cakir97@gmail.com'){return new Request('https://test.invalid/api',{method:body?'POST':'GET',headers:{'Content-Type':'application/json','oai-authenticated-user-id':email,'oai-authenticated-user-email':email},...(body?{body:JSON.stringify(body)}:{})});}
(async()=>{
 ...
 const job=(await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json()).jobs[0],baseline=structuredClone(job.metadata.approvedBudget);
 ...
 const payload=await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json();assert.equal(payload.shifts[0].status,'Completed');assert.deepEqual(payload.jobs[0].metadata.approvedBudget,baseline);
