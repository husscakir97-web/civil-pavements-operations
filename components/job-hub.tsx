...
import {ApprovedBaseline} from '@/components/approved-baseline';
...
export function JobHub({onNavigate}:{onNavigate:(label:NavLabel)=>void}){
 ...
 return <section className="space-y-5">
  ...
  {!error&&(!restored||!data||(jobId&&data.job?.id!==jobId))?<p role="status" className="p-6 text-sm text-slate-500">Loading project…</p>:!jobId?<div className="rounded-xl border border-dashed bg-white p-6"><h2 className="font-semibold">Choose a project to begin</h2><p className="mt-1 text-sm text-slate-600">Select an awarded project to view its tender baseline, readiness, delivery records, commercial position and closeout evidence in one place.</p></div>:data?.job&&data.job.id===jobId&&<>
   ...
   <p className="text-sm text-slate-500">Projects › {data.job.name} › {tab}</p><nav aria-label="Project workspace" className="flex gap-2 overflow-x-auto border-b pb-3">{tabs.map(t=><Button key={t} className="shrink-0" variant={tab===t?'default':'outline'} onClick={()=>setTab(t)}>{t}</Button>)}</nav>
   ...
   {tab==='Setup'&&<div className="grid gap-4 lg:grid-cols-2">
    <section className="rounded-xl border bg-white p-4"><h3 className="font-semibold">Tender handover & approved baseline</h3><p className="mt-1 text-sm text-slate-500">The awarded estimate remains the commercial baseline for the project.</p><ApprovedBaseline value={data.job.metadata?.approvedBudget}/></section>
