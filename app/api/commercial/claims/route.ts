...
const action=z.discriminatedUnion('action',[
 z.object({action:z.literal('create'),projectId:z.string(),period:z.string(),claimDate:day.nullable().optional(),notes:z.string().max(5000).nullable().optional(),lines:z.array(z.object({lineType:z.enum(['contract','variation','docket','other']),sourceId:z.string().nullable().optional(),description:z.string().max(500).optional(),thisClaim:z.coerce.number().min(-1e12).max(1e12)})).max(500)}),
...
export const POST=api({permission:'write',module:'commercial'},async({request})=>{
 ...
 switch(b.action){
  case 'create':return Response.json(await createClaim(b.projectId,{period:b.period,claimDate:b.claimDate,notes:b.notes,lines:b.lines}),{status:201});
