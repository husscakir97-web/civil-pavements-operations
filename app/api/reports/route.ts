...
import { currentOrganisationId, requireEstimateDb, safeJson, jsonError } from '@/lib/estimates-db';
...
async function handleGET(request: Request) {
 try {
  ...
  const entries=await Promise.all(reportTables.map(async table=>{
   ...
   const result=await db.prepare(`SELECT ${columns} FROM ${table} WHERE organisation_id=?`).bind(currentOrganisationId()).all<Record<string,unknown>>();
...
export const GET=withActor(handleGET,'read','reports');
