...
export async function today(days=7){
 ...
 const shifts=await query("SELECT s.id,s.name,s.status,s.metadata,s.created_at,j.id AS job_id,j.name AS job_name,j.project_number,j.stage AS job_stage,j.status AS job_status,j.metadata AS job_metadata,j.site_address FROM shifts s LEFT JOIN jobs j ON j.id=JSON_UNQUOTE(JSON_EXTRACT(s.metadata,'$.jobId')) AND j.organisation_id=s.organisation_id WHERE s.organisation_id=? AND JSON_UNQUOTE(JSON_EXTRACT(s.metadata,'$.date'))>=? AND JSON_UNQUOTE(JSON_EXTRACT(s.metadata,'$.date'))<=? AND s.status NOT IN ('Cancelled','Archived','Draft') ORDER BY JSON_UNQUOTE(JSON_EXTRACT(s.metadata,'$.date')),JSON_UNQUOTE(JSON_EXTRACT(s.metadata,'$.start'))",[org,start,end]);
 ...
 const out=shifts.map(s=>{
  ...
  const projectSwms=swms.filter(w=>w.project_id===s.job_id).map(w=>({id:w.id,reference:w.reference,title:w.title,activity:w.activity,acknowledged:Number(w.mine)>0}));
  const job=safeJson<Row>(s.job_metadata,{});
  return {id:s.id,name:s.name,status:s.status,date:m.date,start:m.start,finish:m.finish,location:m.location||s.site_address||job.site||'',supervisor:m.supervisor||'',activity:m.scope||job.scope||'',instructions:m.instructions||'',preStart:m.preStart||'',siteContact:m.siteContact||'',crew:assignments.map(x=>({name:x.name,role:x.role,category:x.category})),
...
export type FieldDocketInput={shiftId:string;docketNo:string;workDate:string;labourHours:number;quantity:number;quantityUnit:string;notes:string;lines:Array<{description:string;quantity:number;unit:string}>};
...
export async function submitFieldDocket(input:FieldDocketInput){
 ...
 return tx(async conn=>{
  const shift=await one('SELECT id,name,metadata FROM shifts WHERE organisation_id=? AND id=?',[a.organisationId,input.shiftId],conn);
  if(!shift)fail(404,'Shift not found.');
  const jobId=String(safeJson<Row>(shift!.metadata,{}).jobId||'');
  ...
  if(job!.stage==='closed')fail(409,'This project is closed. Ask the office to reopen it.');
  ...
  if(dup)fail(409,'A docket with this number already exists for this date.');
  ...
  await exec("INSERT INTO dockets (id,organisation_id,docket_no,work_date,client,project,crew,labour_hours,quantity,quantity_unit,amount,status,notes,confidence,links,line_items,extraction_method,field_confidence,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,0,'review',?,100,?,?,'field-entry','{}',?,?)",
   [id,a.organisationId,docketNo,input.workDate,job!.client_name||String(safeJson<Row>(job!.metadata,{}).client||''),job!.name,(a.name||a.email).slice(0,160),input.labourHours,input.quantity,input.quantityUnit||'item',input.notes.slice(0,1000),JSON.stringify({jobId,shiftId:input.shiftId,submittedBy:a.userId}),JSON.stringify(lines),now,now],conn);
  await audit({event:'docket.submitted',entityType:'docket',entityId:id,projectId:jobId,summary:`Docket ${docketNo} submitted from the field for review`,after:{shiftId:input.shiftId,labourHours:input.labourHours,quantity:input.quantity}},conn);
  return {docketId:id,docketNo,status:'review'};
 });
}
