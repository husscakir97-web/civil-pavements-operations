...
export async function GET(request:Request){try{
 ...
 const row=kind==='docket'?await db.prepare("SELECT id FROM dockets WHERE organisation_id=? AND id=? AND lower(status)!='archived'").bind(actor.organisationId,id).first():await db.prepare("SELECT id FROM attachments WHERE organisation_id=? AND id=? AND json_extract(metadata,'$.kind')='invoice'").bind(actor.organisationId,id).first();
...
export async function POST(request:Request){try{await requireActor(request,requireEstimateDb(),'write');return Response.json({error:'Paid AI scanning requires configured billing and a confirmed price. No scan was started and no charge was made.'},{status:409});}catch(e){return authError(e)}}
