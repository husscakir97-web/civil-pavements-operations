...
export async function POST(request: Request) {
  try {
    ...
    await db.batch([
      ...
      ...requirementCopies.results.map(r=>db.prepare('INSERT OR IGNORE INTO job_ims_items (id,organisation_id,job_id,title,document_type,mandatory,status,source_requirement_id,linked_document_id,metadata,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').bind(crypto.randomUUID(),DEFAULT_ORGANISATION_ID,jobId,String(r.title),'Tender requirement',Number(r.mandatory),'Missing',String(r.id),r.linked_document_id || null,JSON.stringify({sourceDocument:r.source_document,sourcePage:r.source_page}),now,now)),
      db.prepare(
