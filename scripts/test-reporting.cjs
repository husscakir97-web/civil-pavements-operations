...
async function report(){const r=await reports.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}));assert.equal(r.status,200);return r.json();}
(async()=>{
 ...
 assert.equal((await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json()).shifts.length,1);p=await report();assert.equal(p.summary.counts.shifts,1);assert.equal(p.summary.upcomingShifts,1);
 const form=new FormData();form.set('records',JSON.stringify([{docketNo:'REPORT-TEST',workDate:'2026-09-10',status:'review',amount:123,sourceName:'test.csv'}]));form.set('file',new File(['docket,amount\nREPORT-TEST,123'],'test.csv',{type:'text/csv'}));r=await dockets.POST(new Request('https://test.invalid/api',{method:'POST',headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'},body:form}));assert.equal(r.status,200);assert.equal(files.size,1);
