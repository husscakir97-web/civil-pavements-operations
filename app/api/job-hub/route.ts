...
async function handleGET(request:Request){try{
 ...
 const shifts=await db.prepare("SELECT * FROM shifts WHERE organisation_id=? AND JSON_UNQUOTE(JSON_EXTRACT(metadata,'$.jobId'))=? ORDER BY created_at DESC").bind(actor.organisationId,jobId).all<Record<string,unknown>>();
 const dockets=await db.prepare("SELECT id,docket_no AS name,status,work_date,amount FROM dockets WHERE organisation_id=? AND JSON_UNQUOTE(JSON_EXTRACT(links,'$.jobId'))=? ORDER BY work_date DESC").bind(actor.organisationId,jobId).all();
 const variations=await db.prepare("SELECT id,name,status,metadata FROM commercial_records WHERE organisation_id=? AND JSON_UNQUOTE(JSON_EXTRACT(metadata,'$.jobId'))=?").bind(actor.organisationId,jobId).all();
 ...
 const activity=await db.prepare("SELECT id,name,created_at FROM audit_events WHERE organisation_id=? AND (JSON_UNQUOTE(JSON_EXTRACT(metadata,'$.jobId'))=? OR JSON_UNQUOTE(JSON_EXTRACT(metadata,'$.recordId'))=?) ORDER BY created_at DESC LIMIT 100").bind(actor.organisationId,jobId,jobId).all();
