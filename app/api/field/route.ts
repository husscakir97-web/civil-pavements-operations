...
async function handleGET(request:Request){try{
 ...
 return Response.json({user,record:r&&user.role==='field'?fieldRecord(r):r,history:user.role==='field'?history.results.map(h=>{const item=h as Record<string,unknown>;return {...item,snapshot:JSON.stringify(fieldRecord(JSON.parse(String(item.snapshot))))};}):history.results},{headers:{'Cache-Control':'private, no-store'}});
...
async function handlePOST(request:Request){try{
 ...
 const input={...initialField(plan),...body.data};
 const data=user.role==='field'?preserveFieldPricing(input,previous?.data||initialField(plan)):input;
 if(user.role!=='field')data.pricingReviewRequired=!data.materialCost||!data.otherCost||data.resources.some(r=>r.rate===undefined||r.rate==='')||data.variations.some(r=>r.cost===undefined||r.cost==='');
 const errors=body.action==='save'?invalidField(data):incomplete(data,user.role!=='field');
