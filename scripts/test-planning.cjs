...
const request=body=>new Request('https://test.invalid/api',{method:'POST',headers:{'Content-Type':'application/json','oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'},body:JSON.stringify(body)});
(async()=>{
 ...
 let payload=await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json();const job=payload.jobs.find(j=>j.id===jobId);const baseline=structuredClone(job.metadata.approvedBudget);
 ...
 payload=await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json();assert.equal(payload.shifts[0].metadata.assignments[0].resourceId,worker.id);assert.deepEqual(payload.jobs[0].metadata.approvedBudget,baseline);
 ...
 assert.equal((await (await delivery.GET(new Request('https://test.invalid',{headers:{'oai-authenticated-user-id':'test-owner','oai-authenticated-user-email':'huss.cakir97@gmail.com'}}))).json()).jobs.length,1);
