...
export function incomplete(f:FieldData):string[] {
 const errors:string[]=[];
 for(const [k,label] of [['arrival','Site arrival'],['departure','Site departure'],['tonnes','Tonnes (enter 0 if none)'],['loads','Loads'],['trips','Trips'],['area','Area'],['wastage','Wastage'],['materialCost','Actual material cost'],['otherCost','Other actual costs'],['diary','Site diary'],['supervisorName','Supervisor name'],['supervisorSignature','Supervisor signature']] as const)if(!String(f[k]||'').trim())errors.push(label);
 ...
 f.resources.forEach((r,i)=>{if(!r.name||!r.attendance||r.attendance==='Unconfirmed')errors.push(`Resource ${i+1}: confirm attendance / use`);if(!['Absent','Not used'].includes(String(r.attendance))&&(r.rate===undefined||r.rate===''))errors.push(`Resource ${i+1}: enter cost rate (0 if no charge)`);if(!['Absent','Not used'].includes(String(r.attendance))&&((r.hours===''||r.hours===undefined)&&!(r.start&&r.finish)))errors.push(`Resource ${i+1}: actual hours or start / finish`);});
 for(const [key,required] of Object.entries({delays:['start','finish','cause','party'],cycles:['truck','start','finish'],checks:['time','temperature','result'],instructions:['time','representative','details'],standDowns:['start','finish','reason','party'],incidents:['time','type','details','action'],variations:['description','instructionBy','cost']})) (f[key as keyof FieldData] as Row[]).forEach((r,i)=>{if(required.some(k=>r[k]===undefined||r[k]===''))errors.push(`${key} ${i+1}: complete details`);});
 return [...errors,...invalidField(f)];
}
export function invalidField(f:FieldData):string[] {
 const errors:string[]=[];
 for(const key of ['tonnes','loads','trips','area','wastage','materialCost','otherCost'] as const)if(f[key]!==''&&(!Number.isFinite(Number(f[key]))||Number(f[key])<0))errors.push(`${key}: use a non-negative number`);
